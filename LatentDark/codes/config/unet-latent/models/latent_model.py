...
from .modules.loss import MatchingLoss, PerceptualMatchingLoss
...
class LatentModel(BaseModel):
    ...
    def feed_data(self, LQ, R, GT=None):
        self.LQ = LQ.to(self.device)  # LQ
        self.R = R.to(self.device)  # R
    ...
    def optimize_parameters(self, step):
        ...
        L_r, H_r = encode_fn(self.R)
        L_gt, H_gt = encode_fn(self.GT)

        # rec_llq_hlq = decode_fn(L_lq, H_lq) # latent LQ, hidden LQ
        # rec_lr_hr = decode_fn(L_r, H_r) # latent LQ, hidden LQ
        rec_lgt_hlq = decode_fn(L_gt, H_lq, H_r) # latent GT, hidden LQ

        # loss_rec = self.loss_fn(rec_llq_hlq, self.LQ) #+ self.loss_fn(rec_lgt_hgt, self.GT)
        # loss_rec1 = self.loss_fn(rec_lr_hr, self.R)
        loss_rep = self.loss_fn(rec_lgt_hlq, self.GT) #+ self.loss_fn(rec_llq_hgt, self.LQ)

        # loss_reg = torch.mean(L_lq**2) + torch.mean(L_gt**2) 这个注释掉的

        loss_reg = (L_lq.mean()-self.LQ.mean()).abs() + (L_lq.std() - self.LQ.std()*0.5).abs()
        loss_reg1 = (L_r.mean() - self.R.mean()).abs() + (L_r.std() - self.R.std() * 0.5).abs()

        # loss = loss_rec + loss_rep + loss_reg * 0.001 + loss_rec1 + loss_reg1 * 0.001
        loss = loss_rep + loss_reg * 0.001 + loss_reg1 * 0.001
        # loss = loss_rep
        loss.backward()
        self.optimizer.step()
        ...
        self.log_dict["loss_reg1"] = loss_reg1.item()
        # self.log_dict["loss_rec1"] = loss_rec.item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # self.real_r = decode_fn(L_r, H_r)
            # concat_result = torch.cat((H_lq, H_r), dim=-1)
            self.fake_gt = decode_fn(L_gt, H_lq, H_r) # latent GT, hidden LQ

            # self.fake_lq = decode_fn(L_lq, H_gt) # latent LQ, hidden GT
            # self.fake_r = decode_fn(L_r, H_gt)
        ...
        tvutils.save_image(self.R.data, f'image/R.png', normalize=False)
        ...
        # tvutils.save_image(self.fake_r.data, f'image/R_fake.png', normalize=False)
        ...
        # tvutils.save_image(self.real_r.data, f'image/R_real.png', normalize=False)
        ...
        # tvutils.save_image(L_r.data, f'image/R_latent.png', normalize=False)

        def save_latent_image(tensor, name):
            ...
            # Convert tensor to a single image by flattening and tiling
            tensor_np = normalized_tensor.squeeze().cpu().numpy()

            if tensor_np.ndim == 3:  # If the tensor has multiple channels
                # Reshape and normalize to [0, 255]
                c, h, w = tensor_np.shape
                tensor_np = tensor_np.reshape(c * h, w)
                tensor_np = (tensor_np * 255).astype(np.uint8)
            elif tensor_np.ndim == 2:  # If the tensor is already 2D
                tensor_np = (tensor_np * 255).astype(np.uint8)

            img = Image.fromarray(tensor_np)
            img.save(f'image/{name}.png')

        save_latent_image(L_lq, 'LQ_latent')
        save_latent_image(L_gt, 'GT_latent')
        save_latent_image(L_r, 'R_latent')
