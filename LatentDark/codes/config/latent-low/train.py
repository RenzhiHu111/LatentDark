...
def main():
    ...
    parser.add_argument("-opt", type=str,
                        default="./codes/config/latent-low/options/train/lol.yml" ,help="Path to option YMAL file.")
    parser.add_argument(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # print(current_step)
            LQ, GT, R = train_data["LQ"], train_data["GT"], train_data["R"]
            latent_LQ, _ = model.encode(LQ.to(device))
            latent_GT, _ = model.encode(GT.to(device))
            latent_R, _ = model.encode(R.to(device))
            timesteps, states = sde.generate_random_states(x0=latent_GT, mu=latent_LQ)

            model.feed_data(states, latent_LQ, latent_R, latent_GT) # xt, mu, x0
            model.optimize_parameters(current_step, latent_R, timesteps, sde)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    LQ, GT, R = val_data["LQ"], val_data["GT"], val_data["R"]
                    with torch.no_grad():
                        latent_LQ, hidden = model.encode(LQ.to(device))
                        latent_R, hidden1 = model.encode(R.to(device))
                        noisy_state = sde.noise_state(latent_LQ)

                    # valid Predictor
                    model.feed_data(noisy_state, latent_LQ, latent_R, GT)
                    model.test(sde, hidden, hidden1)
