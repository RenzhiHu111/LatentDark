...
class DenoisingModel(BaseModel):
    def __init__(self, opt):
        ...
        # Calculate and log the number of parameters in the internal PyTorch model
        ...
        # input_tensor = torch.randn(1, 3, 256, 256).to(self.device)  # Adjust the input shape according to your model
        # flops = torchprofile.profile_macs(self.model, input_tensor)
    ...
    def feed_data(self, state, LQ, R, GT=None):
        self.state = state.to(self.device)    # noisy_state
        self.condition = LQ.to(self.device)  # LQ
        self.condition1 = R.to(self.device)  # LQ
        if GT is not None:
            self.state_0 = GT.to(self.device)  # GT
        else:
            self.state_0 = None

    def optimize_parameters(self, step, latent_R, timesteps, sde=None):
        sde.set_mu(self.condition)
        sde.set_cond1(self.condition1)
    ...
    def test(self, sde=None, hidden=None, hidden1=None, perform_ode=False, save_states=False):
        sde.set_mu(self.condition)
        sde.set_cond1(self.condition1)
        ...
        with torch.no_grad():
            if not perform_ode:
                ...
                latent = sde.reverse_posterior(self.state, save_states=save_states)
            else:
                # if perform Denoising ODE
                latent = sde.reverse_ode(self.state, save_states=save_states)

            self.output = self.decode(latent, hidden, hidden1)
