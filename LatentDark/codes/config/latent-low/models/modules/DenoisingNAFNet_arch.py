...
from .module_util import SinusoidalPosEmb, LayerNorm, exists, LAM_Module_v2, CBAMBlock
...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], upscale=1):
        ...
        self.layer_fussion = LAM_Module_v2(in_dim=img_channel*2)

        self.intro = nn.Conv2d(in_channels=img_channel*6, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        ...
        for num in dec_blk_nums:
            ...
            self.cbams.append(CBAMBlock(channel=chan, kernel_size=7))
        ...
        # self.weight_factors_x = nn.ParameterList([nn.Parameter(torch.ones(1)) for _ in range(len(self.decoders))])
        # self.weight_factors_enc_skip = nn.ParameterList([nn.Parameter(torch.ones(1)) for _ in range(len(self.decoders))])

    def forward(self, inp, cond, cond1, time):

        if isinstance(time, int) or isinstance(time, float):
            time = torch.tensor([time]).to(inp.device)

        x = inp - cond
        # x = torch.cat([x, cond, cond1], dim=1)
        x = self.layer_fussion(x, cond, cond1)
        ...
        #     x = up(x)
        #     x = self.weight_factors_x[i] * x + self.weight_factors_enc_skip[i] * enc_skip
        #     x, _ = decoder([x, t])
