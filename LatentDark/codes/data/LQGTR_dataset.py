...
class LQGTRDataset(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
        self.LR_paths, self.GT_paths, self.R_paths = None, None, None
        self.LR_env, self.GT_env, self.R_env = None, None, None  # environment for lmdb
        self.LR_size, self.GT_size, self.R_size = opt["LR_size"], opt["GT_size"], opt["R_size"]
    ...
    def __getitem__(self, index):
        if self.opt["data_type"] == "lmdb":
            if (self.GT_env is None) or (self.LR_env is None) or (self.R_env is None):
                self._init_lmdb()

        GT_path, LR_path ,R_path= None, None, None
        ...
        if self.opt["phase"] == "train":
            ...
            img_LR, img_GT , img_R = util.augment(
                [img_LR, img_GT, img_R],
                self.opt["use_flip"],
                self.opt["use_rot"],
                self.opt["mode"],
        ...
        return {"LQ": img_LR, "GT": img_GT, "R": img_R, "LQ_path": LR_path, "GT_path": GT_path, "R_path": R_path}
